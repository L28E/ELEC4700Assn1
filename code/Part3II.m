...
type=1;                 % 0 for specular, anything else for diffusive 
...
smol_width=0.1e-7;
smol_height=0.2e-7;
smol_right=smol_x+smol_width;
smol_top=smol_y+smol_height;

% Generate random electron positions
Px=rand(1,num_electrons).*0.01*-xmax; % Start slightly off screen
...
Vx=abs(randn(1,num_electrons)*sqrt(k_b*T/m_n));
...
for k=2:num_steps
    ...
    % Electrons leaving lateral bounds DO NOT come back in to preserve density
    ...
    % And the smol box. I'll assume it hit the left side.
    oob=(Px>=smol_x & Px<=smol_right & Py<=smol_top & Py>=smol_y);    
    if type==0          
        Vx(oob)=-Vx(oob);       
    else
        Vx(oob)=-sign(Vx(oob)).*abs(randn(1,nnz(oob))*sqrt(k_b*T/m_n)); 
        Vy(oob)=-sign(Vy(oob)).*abs(randn(1,nnz(oob))*sqrt(k_b*T/m_n));
    end    
    Px(oob)=2*smol_x-Px(oob);
